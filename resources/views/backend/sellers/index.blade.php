    ...
    @endcan-->
            ...
            @if (auth()->user()->can('add_seller') && $route == 'all_seller_route')
                <div class="col text-right">
                    ...
                    <a href="{{ route('sellers.bulk_upload') }}" class="btn btn-success ml-2">
                        Import Sellers
                    </a>
                    <a href="{{ route('sellers.export') }}" class="btn btn-secondary ml-2">
                        Export Sellers
                    </a>
                </div>
            @endif
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="row gutters-5  mw-100 align-items-center">
                                        <div class="col-auto">
                                            <img src="{{ uploaded_asset($shop->logo) }}" class="size-40px img-fit"
                                                alt="Image"
                                        ...
                                        <div class="col @if ($shop->user->is_suspicious == 1) text-info @endif">
                                            <span class="text-truncate-2">
                                                ...
                                                <a class="text-primary"
                                                    href="{{ route('sellers.profile', encrypt($shop->id)) }}"
                                                    target="_blank">
                                ...
                                @if ($route == 'all_seller_route')
                                    <td>
                                        ...
                                        @elseif($shop->user->is_suspicious)
                                            <span
                                                class="badge badge-inline badge-info">{{ translate('Suspicious') }}</span>
                                        @elseif(!$shop->user->banned)
                                            <span
                                                class="badge badge-inline badge-success">{{ translate('Regular') }}</span>
                                        @endif
                                    </td>
                                    {{-- <td>{{ $shop->user->products->count() }}</td> --}}
                                    <td>{{ $shop->user->sellerProducts->count() }}</td>
                                    <!--<td>-->
                                    ...
                                    <td>
                                        <div class="d-flex flex-column align-items-start">
                                            @if ($shop->verification_status != 1 && $shop->verification_info != null)
                                                <span class="badge badge-inline badge-warning mb-1">
                                                    {{ translate('Applied') }}</span>
                                                <a href="javascript:void();"
                                                    onclick="show_seller_verification_info('{{ $shop->id }}');"
                                                    class="badge badge-inline badge-info">
                                            ...
                                            @elseif($shop->verification_status == 1 && $shop->verification_info == null)
                                                <span class="badge badge-inline badge-success mb-1">
                                                    {{ translate('Verified') }}</span>
                                                <span
                                                    class="badge badge-inline badge-secondary">{{ translate('By Admin') }}</span>
                                            @else
                                                <span class="badge badge-inline badge-secondary">
                                                    {{ translate('Not Applied') }}</span>
                                            @endif
                                        </div>
                                    </td>
                                    <td>
    ...
    <div class="modal fade" id="edit_seller_custom_followers">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <form class="form-horizontal" action="{{ route('edit_Seller_custom_followers') }}" method="POST"
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-9">
                                <input type="number" lang="en" min="0" step="1"
                                    placeholder="{{ translate('Custom Followers') }}" value=""
                                    name="custom_followers" id="custom_followers" class="form-control" required>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="submit"
                            class="btn btn-primary btn-sm text-white">{{ translate('save!') }}</button>
...
@section('script')
    <script type="text/javascript">
        ...
        function show_seller_verification_info(id) {
            $.post('{{ route('sellers.verification_info_modal') }}', {
                _token: '{{ @csrf_token() }}',
                id: id
            }, function(data) {
                $('#verification_info_modal #verification-info-modal-content').html(data);
                $('#verification_info_modal').modal('show', {
                    backdrop: 'static'
                });
            });
        }
        ...
        function showConfirmationModal({
            ...
            // Show the modal
            $('#universal-confirm-modal').modal('show', {
                backdrop: 'static'
            });
        }
        ...
        function set_bulk_commission() {
            ...
            if (sellerIds.length > 0) {
                ...
                $('#set_seller_commission').modal('show', {
                    backdrop: 'static'
                });
            } else {
        ...
        function editCustomFollowers(shop_id, custom_followers) {
            ...
            $('#edit_seller_custom_followers').modal('show', {
                backdrop: 'static'
            });
        }
        ...
        function confirm_suspicious(url, isSuspicious) {
            ...
            showConfirmationModal({
                url: url,
                message: '{{ translate('Do you really want to') }} ' + action +
